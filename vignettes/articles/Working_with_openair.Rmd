...
The `wrcc_leaflet()` function provides an interactive map to help find the ID 
associated with a particular location. Just click on a dot to find the associated
ID:

```{r wrcc_leaflet}
# NOTE:  wrcc_leaflet() expects 'stationID' rather than 'nwsID'
meta$stationID <- meta$nwsID
wrcc_leaflet(meta)
```

We will shoose the station in Enumclaw, Washington (451702) and create a 
