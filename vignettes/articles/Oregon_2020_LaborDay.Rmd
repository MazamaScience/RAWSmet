...
if ( file.exists(filePath) ) {
  ...
  # Iterate through every station in oregon and download its data
  for ( wrccID in oregonMeta$wrccID ) {
    ...
    res <- try({
      ...
      # Load/download station data and filter by the important dates
      ...
      # Add the station timeseries object to a list
...
`RAWSMet::addWindBarbs()`, show the direction and speeds of *wind gusts* at 
...
```{r windRosesOGOO, echo = TRUE, message = FALSE, fig.width = 5, fig.height = 5, fig.show = 'hold'}
...
wrccPlot <- openair::windRose(mydata = orOGOOData,
                              ws = "windSpeed",
                              wd = "windDirection",
