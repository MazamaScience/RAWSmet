...
waList <- waList %>%
  ...
  rawsList_removeEmpty()
```
While there are 205 stations in Oregon, only 135 of them contain data in this
time range.
...
gg <- 
  ...
  # Change the color of each point depending on where it falls in 'breaks'
  scale_color_stepsn(
    name = "Precipitation (mm)",
    breaks = breaks,
    colors = colors,
    values = values,
    # This is why we set 0 values to NA
    na.value = "black",
    guide = guide_colorsteps(order = 1)
  ) +
  # Add 0 values to the legend
  geom_point(aes(size = "No Precipitation"), shape = NA, colour = "black") +
  guides(size = guide_legend("", override.aes=list(shape=15, size = 7), order = 2)) +
  ggtitle(
    label = "Total Precipitaion in Washington and Oregon \n Aug 01 - Sep 07, 2020"
  ) +
